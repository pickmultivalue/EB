...
!
    EB.PROMPT=FG$TLINE:':':FG$TACC.NAME:'>'
    COL=LEN(EB.PROMPT)+1; PROW=22
    POST.PROMPT=FALSE
    EB.PROMPT=@(0,PROW):BG:EB.PROMPT:FG
    SAVE.COLOURS=FG$CURR.COLOURS:AM:FG$PREV.COLOURS
    IF WHITE<1,1>#'' THEN CALL EB_CH_COLOUR(FG$COLOURS<1,4,1>,FG$COLOURS<1,4,2>)
    FG$CURR.COLOURS=SAVE.COLOURS<1>; FG$PREV.COLOURS=SAVE.COLOURS<2>
    IF GUI THEN
        CALL EB_ERRMSG(FG$ERROR.MSGS<83>, 0)
        CALL EB_GUI_SEND('r','')
        PROMPT ''
    END ELSE CRT @(0,PROW):CLEOP:@(0,21):FG$ERROR.MSGS<83>
...
!    END
    ...
    UNTIL UCOMMAND='OFF' OR COMMAND='.' OR FG$ACT.CODE=FG$ABT.CODE OR FG$ACT.CODE=FG$END.CODE DO
    ...
    FG$ACT.CODE=FALSE
    CRT FG$CURR.COLOURS:
    IF INDEX('ZR',FG$TERM.TYPE,1) THEN FG$CRT.PAGES=''; FG$CRT.SEQUENCES=''
    IF MOD(FG$STERM,3) THEN
        CALL EB_AT_WINDOW_CLOSE(1)
    END
    RETURN
...
!
    LOCATE PROCESS.ID IN FG$SESSIONS<1,vm_start> BY 'AL' SETTING POS THEN
        LPOS=FG$SESSIONS<2,POS>
        DEL FG$SESSIONS<1,POS>
        DEL FG$SESSIONS<2,POS>
        LOCATE LPOS IN FG$SESSIONS<2,vm_start> SETTING POS ELSE CALL EB_RESET(LPOS)
    END
    RETURN
!
!----------------------------------------------------------------------
