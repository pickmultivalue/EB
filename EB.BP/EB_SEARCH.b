...
!
        ...
        WHILE POS DO
            BEGIN CASE
                CASE FG_ACT.CODE=FG_OPT.CODE
                    CALL EB_CHOICES(50,3,31,30,'',SSS,Z,1,RPOS,1,'L#30','Previous Searches')
                    SCR.UD = 1
                    CALL EB_REFRESH
                    GOSUB DisplayPrompt
    ...
    IF SSTR='///' OR SSTR='\\\' THEN
        ...
        END
    END
    IF SSTR EQ ESC OR FG_ACT.CODE EQ FG_ABT.CODE THEN GO 4096
...
4120 !
    ...
    Y=Z
    CRT MSG.DSP:
    IF Y EQ ESC OR FG_ACT.CODE EQ FG_ABT.CODE THEN GO 4199
