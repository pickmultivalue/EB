...
* @(#) EB_UT_WP.b Ported to jBASE 07:23:52  18 FEB 2010
...
!
                IF INSERTING THEN
                    IF TRIM(INP.STRING[LENTH,1])#'' OR INP.POS-1=LENTH THEN
                        CRT BELL:
                    END ELSE
                        INP.STRING=(INP.STRING[1,INP.POS-1]:WPCHR:INP.STRING[INP.POS,LENTH])
                        WPCOL+=1
                        INP.POS+=1
                        DUMMY=TRIM(INP.STRING[INP.POS,LENTH],' ',"T")
                        IF DUMMY#'' THEN GOSUB CRT.UNDERLINE
                    END
                END ELSE
                    IF INP.POS-1=LENTH THEN
                        CRT BELL:
                    END ELSE
                        IF INP.POS=1 AND TRIM(INP.STRING)#'' THEN
                            ...
                            END
                        END
                        INP.STRING=(INP.STRING[1,INP.POS-1]:WPCHR:INP.STRING[INP.POS+1,LENTH])
                        WPCOL+=1
                        INP.POS+=1
                    END
                END
...
! INS char
        CASE SUB.CODE=FG$INS.CODE
            IF INDEX(' ',INP.STRING[LENTH,1],1) THEN Sub=5 ELSE CRT BELL:
! Set INS
...
! INS Line
        ...
        CASE SUB.CODE=FG$PRVS.CODE OR SUB.CODE=FG$NXTS.CODE
            IF NOT(NORMAL.FIELD) THEN
                ...
                GO FINISH
            END
        CASE WPCHR.NBR>=FIRST.ASCII AND WPCHR.NBR<=LAST.ASCII AND INP.POS-1<=LENTH
            GO ASCII.INPUT
        END CASE
        IF Sub THEN
...
CRT.UNDERLINE:      !
    ...
    INCLUDE EB.OS.INCLUDES PC.OFF.CURSOR
    STMP=TRIM(INP.STRING[INP.POS,LENTH],' ',"T")
    XX=LEN(STMP)
...
CRT.LINE: !
    ...
    RETURN
NEXT.WORD:!
    IF TRIM(INP.STRING[INP.POS,LENTH])#'' THEN
        WPCOL-=INP.POS
        LOOP
            THIS.CHR=INP.STRING[INP.POS,1]
        UNTIL OCONV(THIS.CHR,'MCN')='' AND OCONV(THIS.CHR,'MCA')='' OR THIS.CHR=' ' OR INP.POS>=LENTH DO
            INP.POS+=1
        REPEAT
        WPCOL+=INP.POS
        THIS.CHR=TRIM(INP.STRING[INP.POS,LENTH])
        IF THIS.CHR#'' THEN
            WPCOL-=INP.POS
            LOOP
                THIS.CHR=INP.STRING[INP.POS,1]
            UNTIL OCONV(THIS.CHR,'MCN')#'' OR OCONV(THIS.CHR,'MCA')#'' OR TRIM(INP.STRING[INP.POS,LENTH])='' OR INP.POS>=LENTH DO
                INP.POS+=1
            REPEAT
            WPCOL+=INP.POS
        END
...
GET.WORD: !
    ...
    UNTIL THIS.CHR=' ' OR (OCONV(THIS.CHR,'MCN')='' AND OCONV(THIS.CHR,'MCA')='' AND THIS.CHR#"'") OR NEW.POS>LENTH DO
        NEW.POS+=1
    REPEAT
...
5         !
    WPCHR=' '
    INP.STRING=INP.STRING[1,INP.POS-1]:' ':INP.STRING[INP.POS,LENTH]
    GOSUB CRT.UNDERLINE
    IF INP.POS<=PASTE THEN
...
7         !
    ...
    IF WPCHR.NBR=SEQ(FG$BS.CH) THEN
        CRT SPC:BACK:
        TRAIL=1
        INP.STRING=INP.STRING[1,INP.POS-1]:INP.STRING[INP.POS+1,LENTH]
        GOSUB CRT.UNDERLINE
    END
    IF NOT(INSERTING) THEN
        IF PASTE THEN
...
DEL.LABEL:!
    INP.STRING=(INP.STRING[1,INP.POS-1]:INP.STRING[INP.POS+1,LENTH])[1,LENTH]
...
9         !
    ...
    INP.STRING=(INP.STRING[1,INP.POS-1]:INP.STRING[NEW.POS,LENTH])    ;! JUST
    TRAIL=WORD.LENGTH+1
    ...
    RETURN
10        !
    TRAIL=TRIM(INP.STRING[INP.POS,LENTH],' ',"T")
    FG$DELETE.LIST=INP.STRING[INP.POS,LENTH]:AM:FG$DELETE.LIST
    TRAIL=LEN(TRAIL)
...
18        !
    ...
    ST=OLD.POS; FI=INP.POS-1; GOSUB FOREGROUND
    INP.STRING=(INP.STRING[1,INP.POS-1]:WORD:INP.STRING[NEW.POS,LENTH])         ;! JUST
    IF PASTE THEN
...
INSERT.STRING:      !
    ...
    LOOP
        ...
        INP.STRING=TRIM(INP.STRING[1,INP.POS-1]:PASTE.STRING[1,WORD.LENGTH]:INP.STRING[INP.POS,LENTH],' ',"T")
