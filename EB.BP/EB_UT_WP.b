...
!
        IF NOT(FG_ACT.CODE) THEN
            ...
            IF (OCONV(STMP,'MCP')=STMP AND (OCONV(STMP,'MCA')=STMP OR OCONV(STMP,'MCN')=STMP)) OR TRIM(STMP)='' ELSE
                IF FG_MAX.CHARS<3 AND OCONV(LAST.CHR,'MCA') NE '' AND PREV.CHARS='' THEN IF WPCHR.NBR>=FIRST.ALPHA AND WPCHR.NBR<=LAST.ALPHA THEN GO ASCII.START
...
!
                IF INSERTING AND NOT(FIRST.TIME) THEN
                    ...
                    END ELSE
                        ...
                        IF DUMMY NE '' THEN GOSUB CRT.UNDERLINE
                    END
                END ELSE
                    IF INP.POS-1=MAXLENTH THEN
                        CRT BELL:
                    END ELSE
                        IF INP.POS=1 AND TRIM(INP.STRING) NE '' THEN
...
CRT.UNDERLINE: !
    ...
    INCLUDE EB.OS.INCLUDES PC.OFF.CURSOR
    STMP=TRIM(INP.STRING[INP.POS,LENTH-WPCOL-1],' ',"T");! JUST
    XX=LEN(STMP)
...
CRT.LINE: !
    ...
    RETURN
NEXT.WORD:
    IF TRIM(INP.STRING[INP.POS,MAXLENTH]) NE '' THEN
        WPCOL-=INP.POS
        LOOP
        ...
        IF THIS.CHR NE '' THEN
            ...
            LOOP
                THIS.CHR=INP.STRING[INP.POS,1]
            UNTIL OCONV(THIS.CHR,'MCN') NE '' OR OCONV(THIS.CHR,'MCA') NE '' OR TRIM(INP.STRING[INP.POS,MAXLENTH])='' OR INP.POS>=MAXLENTH DO
                INP.POS+=1
            REPEAT
            WPCOL+=INP.POS
        END
...
GET.WORD: !
    ...
    UNTIL THIS.CHR=' ' OR (OCONV(THIS.CHR,'MCN')='' AND OCONV(THIS.CHR,'MCA')='' AND THIS.CHR NE "'") OR NEW.POS>MAXLENTH DO
        NEW.POS+=1
    REPEAT
...
3   !
    ...
    INP.POS=LEN(DUMMY)
    IF INP.POS GT LENTH THEN INP.POS = LENTH
    CRT STR(FWD,INP.POS-WPCOL):;!+1):
!    IF PASTE THEN
...
!    GOSUB 23
    INP.POS += 1
    WPCOL=INP.POS;!ORIG.COL+INP.POS-1
    RETURN
4   !
    IF WPCOL EQ 0 THEN RETURN
    CRT STR(BACK,WPCOL):
    OLD.POS=INP.POS
    WPCOL=ORIG.COL
    INP.POS=1
    STMP=TRIM(INP.STRING[1,MAXLENTH],' ',"T")
    CRT STMP JUST:
    CRT STR(BACK,LENTH):
    IF PASTE THEN
...
9   !
    THIS.CHR=INP.STRING[INP.POS,1]
    IF OCONV(THIS.CHR,'MCA') NE '' OR OCONV(THIS.CHR,'MCN') NE '' THEN
        GOSUB GET.WORD
    END ELSE
        NEW.POS=INP.POS+1
        WORD=THIS.CHR
...
22  !
    RETURN
23  !
    IF INP.POS LT LENTH THEN
        CRT INP.STRING[INP.POS,1] 'L#1':
        WPCOL++
    END ELSE
        CRT STR(BACK,LENTH-1):
        STMP=TRIM(INP.STRING[INP.POS-LENTH+2,MAXLENTH],' ',"T")
        CRT STMP JUST:BACK:
    END
    INP.POS++
    RETURN
...
STMP.OCONV:
    ...
    BEGIN CASE
        ...
        CASE 1
            STMP=INP.STRING
    END CASE
    IF update_val THEN INP.STRING = STMP
    RETURN
STMP.ICONV:
    BEGIN CASE
        CASE TYPE='T'
