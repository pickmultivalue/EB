...
!
    ...
    RSTR=Z
    CRT MSG.AKN:
    IF RSTR EQ ESC OR FG_ACT.CODE EQ FG_ABT.CODE THEN GO 3099
    RSTRL=LEN(RSTR)
    ...
    IF NOT(ReplMode) THEN
        ...
        IF LEN(rdelim)=1 THEN
            IF INDEX('sr', RSTR[1,1], 1) AND COUNT(RSTR,rdelim)=1 THEN OK=FALSE
        END ELSE OK=FALSE
        IF NOT(OK) THEN
...
3020    !  Get Start Line
        ...
        STRT=Z
        CRT MSG.AKN:
        IF STRT EQ ESC OR FG_ACT.CODE EQ FG_ABT.CODE THEN GO 3099
...
3030    ! Get End Line
        ...
        IF ENDL EQ ESC OR FG_ACT.CODE EQ FG_ABT.CODE OR NOT(NUM(ENDL)) OR ENDL<STRT THEN GO 3030
3039    !
