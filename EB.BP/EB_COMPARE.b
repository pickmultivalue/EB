    SUBROUTINE EB_COMPARE(FIL,REC,CHANGED,MREC,POS,READ.AGAIN,LCOL,LROW,ROW,INDROW,PR,MSG.CLR,MSG.AKN,FLNM,MFLNM,ITNM,MITNM,DCT,MDCT)
    INCLUDE JBC.h
...
5010 CRT MSG.CLR:"Compare from file "
    ...
    IF FG_ACT.CODE EQ FG_OPT.CODE THEN
        VALIDATE=''; VALIDATE<5>=1
        CALL EB_CHOICES(50,5,'','','', 'OBJECT':@VM:FLNM, IDATA,1,VALIDATE,1,'L#30','')
    END
    IF FG_ACT.CODE EQ FG_ABT.CODE OR MFLNM EQ ESC THEN GO 5090
    ...
    END
    CRT MSG.AKN:
    IF MFLNM EQ FLNM AND POS EQ 1 THEN        ;! default was accepted
        MFL=FIL
    END ELSE
...
!
    IF MFLNM EQ FLNM AND MITNM EQ ITNM AND POS EQ 1 THEN   ;!merge from memory.
