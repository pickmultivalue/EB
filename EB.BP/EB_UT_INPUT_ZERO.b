    SUBROUTINE EB_UT_INPUT_ZERO(CHR,MAT EB_CHARS,FG_ACT.CODE,COL,ROW,CODES,SUBCODES,MAX.SUB,FG_TIMEOUT)
!
    DIM EB_CHARS(100)
...
!
    ...
    FG_MONITOR.SECS=FG_TIMEOUT<2>; FG_TIMEOUT=FG_TIMEOUT<1>; IF FG_MONITOR.SECS='' THEN FG_MONITOR.SECS=FG_TIMEOUT
...
!
    ...
    FG_ACT.CODE=FALSE
    CODE=FALSE
    IF LEN(COL:ROW) THEN CRT @(COL,ROW):
    CRT DFLT.CHR:
    IF LEN(COL:ROW) THEN CRT CHAR(8):
    LOOP
        IF LEN(COL:ROW) THEN CRT @(COL,ROW):
        INCLUDE EB.OS.INCLUDES UT.INPUT.TIMEOUT
        IF FG_TIMEDOUT THEN GO FINISH
        IF (CHR.NBR<FIRST.ASCII OR CHR.NBR>LAST.ASCII) AND COUNT(CODES,CHR)>1 THEN
            ...
            WHILE SYSTEM.14 AND DIFF<=0 DO
                ...
                IF FG_EXPECT.CR AND CHR=CR THEN CHR=''; CHR.NBR=0
                INCLUDE EB.OS.INCLUDES SYSTEM.12
            REPEAT
        END
        PREV.CHARS=(PREV.CHARS:CHR) MAX.JUST
        CHR1=''
        FOR I=FG_MAX.CHARS TO 1 STEP -1 UNTIL FG_ACT.CODE
