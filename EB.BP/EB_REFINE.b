...
!
    ...
    END ELSE
        obj = new object()
    END
    IF cache->$hasproperty('lookup') THEN
        lookupobj = cache->lookup
        detail_routine = lookupobj->detail_routine
    END ELSE
        detail_routine = ''
    END
    ...
    LOOP
        LINE=''
        IF DISPLAY.LIST<1, MV> NE '' THEN
            IF detail_routine NE ''  THEN
                CALL @detail_routine(lookupobj, DISPLAY.LIST, MV)
            END
            FOR J=1 TO NBR.ATTS
                ATTR=ATTRS<1,J>
