...
!
    ...
    IF NO.I.L>0 THEN
        I=LROW+(LROW<3)
        LOOP WHILE I>2 AND (LEN(TRIM(REC<I-1+INDROW-1>)) EQ 0 OR REC<I-1+INDROW-1>[1,COMMENTLEN] EQ COMMENT) DO I-=1 REPEAT
        IF I>1 THEN CHECK.LINE=REC<I-1+INDROW-1> ELSE CHECK.LINE=REC<INDROW-1>
        LLEN=LEN(CHECK.LINE)
