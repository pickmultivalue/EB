    ...
    IF RegInput THEN
        ...
        IF LEN(INP.STRING) THEN
            ...
            IF POS THEN
                BEGIN CASE
                CASE POS=1; FG_ACT.CODE = FG_ABT.CODE
                CASE POS=3; FG_ACT.CODE = FG_EXIT.LN.CODE
                CASE POS=8; FG_ACT.CODE = FG_OPT.CODE
                CASE POS=15; FG_ACT.CODE = FG_HLP.CODE
                END CASE
            END
        ...
        FG_TYPEAHEAD.BUFF = INP.STRING:CR
...
!    END
    ...
    IF INP_FLD<1> # 'LIT' THEN INP.STRING = TRIM(INP.STRING, ' ', 'T')
    IF NOT(GETENV('EB_DEBUG',FG_DEBUG)) THEN FG_DEBUG = FALSE
    IF FG_DEBUG THEN DEBUG
    IF FG_TIMEDOUT THEN
        FG_TIMEDOUT = FALSE
