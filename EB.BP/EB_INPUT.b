...
IF COUNT(FG_INPUT.CODES,PREV.CHARS:CHR)>1 AND CHR.NBR#RTN.VAL AND (CHR.NBR<FIRST.ASCII OR CHR.NBR>LAST.ASCII) THEN
!
...
IF FG_MAX.CHARS<3 AND OCONV(LAST.CHR,'MCA')#'' AND PREV.CHARS='' THEN IF CHR.NBR>=FIRST.ALPHA AND CHR.NBR<=LAST.ALPHA THEN GO ASCII.START
...
END
IF FG_ACT.CODE THEN
IF FG_ACT.CODE=FG_HLP.CODE AND HELP.ID#'' THEN
